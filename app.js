...
var path = require('path');//处理路径的 path.join
...
//flash是依赖session，因为flash中间件在放在使用session中间件的下面
//req.flash
//使用flash中间件放在session中间件下面使用
app.use(flash());
//把flash中的消息，取出来赋给模板变量
app.use(function(req,res,next){
  res.locals.keyword='';
  //res.locals 才是真正的渲染模板的对象
  //可以通过user是否有值来判断此用户是否登录
