...
router.post('/reg',function(req,res){
    //接收请求体，然后保存到数据库中
    //客户端填写注册表单后，点击提交按钮的时候，会把表单内容序列化成查询字符串的格式放在请求体中传到后台body-parser中间件把它转成对象并赋给req.body
    var user=req.body;
    console.log(user);
    if(user.password!=user.repassword){
        req.flash('error','密码和重复密码不一致');
        return res.redirect('back');//表示返回上一页面
    }
    delete user.repassword;
    user.password=md5(user.password);//对密码进行加密
    user.avatar='https://secure.gravatar.com/avatar/'+md5(user.email)+'?s=48';//生成头像图片地址
    //保存一个文档
...
//登录 当用户访问此路径的时候返回一个空白表单
router.get('/login',function(req,res,next){
    //第一个是相对路径，相对于views也就是模板根目录
